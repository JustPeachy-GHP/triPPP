  ...
  useEffect(() => {
    async function getMyData() {
      try {
        ...
        const location_id = extData.location_id;
        const place_id = extData.place_id;
        console.log(extData)
        setMyTrip(extData);
        ...
        <div>
          ...
          {myTrip.isdecided === true ? (
            <button
              ...
              onClick={() =>
                navigate(`/trips/${trip_id}/itinerary/${myTrip.place_id}`)
              }
            >
              {tripname}
              <p>Look at things we can do on this trip.</p>
            </button>
          ...
          myId === myTrip.admin_id ? (
            <button
              className="textButtons"
              onClick={() =>
                navigate(`/trips/${trip_id}/locations`, { replace: true })
              }
            >
              {tripname}
              <p>Let's set the destination for this trip!</p>
            </button>
          ) : (
            <button
              className="textButtons"
              onClick={() =>
                alert(
                  `Waiting for ${myTrip.admin_name} at ${myTrip.admin_email} to set the destination...`
                )
              }
            >
              {tripname}
              <p>
                ... Waiting for{" "}
                {myTrip.admin_name
                  ? `${myTrip.admin_name} at ${myTrip.admin_email} `
                  : `your trip organizer `}{" "}
